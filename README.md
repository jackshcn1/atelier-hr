...
## Update: page-level login protection (no database change needed)
New file: `middleware.js` at the project root (same level as `package.json` — not inside `app/`). This is the piece that was missing: before this, RLS correctly hid all your *data* from a logged-out visitor, but the page shells themselves — forms, layout, navigation — still loaded for anyone. Now, visiting any page without a valid login redirects straight to `/login`.

No SQL migration needed — just add `middleware.js` to your GitHub repo (root level) and redeploy. Test it by logging out and trying to visit `/employees` directly — it should bounce you to `/login` immediately instead of showing an empty page.

## Update: bulk employee import (no database change needed)
New page: `/employees/bulk-import` (linked from the Employees page). Meant for one-time migration of your existing team.

How it works:
1. Click "Download template" for a spreadsheet with the exact column names expected, plus an example row.
2. Fill it in — one row per employee. `department` must exactly match a department name you've already created in Settings; `reporting_manager_id` is optional and should be another row's `employee_id` (order in the file doesn't matter — managers are wired up in a second pass after everyone's created).
3. Upload it, click Validate to see any issues highlighted per row (missing required fields, IDs that already exist, departments that don't exist yet) before anything is written.
4. Click "Import valid rows" — rows with real errors are skipped automatically; everything else gets created with starting salary history and deposits, same as adding someone one at a time.

No SQL migration needed — this only uses your existing tables. Just add the new file to GitHub and redeploy.

## Migration: fix department name relationships
Run this once on your existing database — it lets you rename a department (changes cascade automatically to every employee, document template, and profile using it) and lets you delete a department without being blocked (anyone using it just gets their department cleared, rather than the delete failing outright).
...
alter table doc_templates drop constraint doc_templates_department_fkey;
alter table doc_templates add constraint doc_templates_department_fkey
  foreign key (department) references departments(name) on update cascade on delete set null;

alter table profiles drop constraint profiles_department_fkey;
alter table profiles add constraint profiles_department_fkey
  foreign key (department) references departments(name) on update cascade on delete set null;
```

**If a constraint name doesn't match** (Postgres sometimes names them slightly differently), check Supabase → Database → Tables → the relevant table → its foreign keys for the actual name, and use that instead.

After this migration: to fix a typo'd department name, just rename it (`update departments set name = 'Correct Name' where name = 'Typo Name'`) rather than deleting and recreating — every employee, template, and profile using it updates automatically. Deleting a department that's still in use will now succeed, but clears the department field for anyone who had it, so check who's affected first if that's not what you want.

## Migration: expanded personal details + sensitive info table
Run this once on your existing database:

```sql
alter table employees add column passport_photo_url text;
alter table employees add column gender text;
alter table employees add column address text;
alter table employees add column previous_work_history text;
alter table employees add column education_history text;
...
(Emergency contact, date of birth, blood group, and ID proof type were already columns on `employees` from the original schema — they just didn't have form fields until now.)

### What's new
- Each employee's page now has a **Personal details** section (name, phone, DOB, gender, blood group, address, emergency contact, ID proof type, previous work history, education history, notes) plus a passport-photo upload that shows as a thumbnail next to their name.
- A separate **Sensitive info** section, visually marked and admin-only: ID proof number, PF/ESI numbers (only shown if that employee has PF/ESI applicable), and full bank details — plus ID proof front/back photo upload with thumbnails. Department heads will see a permission error if they try to save this section, same pattern as the payroll settings page.
- One caveat worth knowing: ID proof photos and the passport photo are stored in the same `documents` storage bucket as resignation letters, which currently grants any logged-in user read/write access to the whole bucket — so while the *database record* of sensitive info is admin-only, the *uploaded image files* aren't yet department-scoped at the storage level. Flagging this as a known gap for a future refinement rather than something silently assumed safe.
- Bulk import now supports **updating existing employees**, not just creating new ones — a row with an `employee_id` that already exists updates that person instead of being skipped, and only touches cells that actually have a value (blank cells never overwrite existing data). Useful for something like "set standard hours for everyone" as a one-column bulk update. Salary changes still aren't done this way — that goes through "Record salary change" on the employee's page so it stays properly logged.

## Migration: lock document storage down to admin-only
This tightens something that was left open earlier — the `documents` storage bucket (resignation letters, general documents, ID proofs, passport photos) was readable/writable by any logged-in user. Run this once to make it admin-only across the board:
...
**Known trade-off**: this means a department head currently can't upload a resignation letter, CV, or any document for their own team — only an admin can, even though the underlying database record permissions would otherwise allow it. That's deliberate for now, until we work out exactly what document access department heads should have (a separate conversation). The app's error messages now say "(only admins can upload files right now)" when this blocks someone, rather than showing a raw database error.
