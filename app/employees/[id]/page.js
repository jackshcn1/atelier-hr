...
export default function EmployeeDetail() {
  ...
  const [petpoojaCode, setPetpoojaCode] = useState('');
  ...
  async function load() {
    ...
    if (emp) {
      ...
      setPetpoojaCode(emp.petpooja_employee_code || '');
  ...
  async function savePayrollInfo(e) {
    ...
    const { error } = await supabase.from('employees').update({
      petpooja_employee_code: petpoojaCode || null,
    ...
    <div>
      ...
      <section style={{ background: 'white', padding: 16, borderRadius: 8, marginTop: 20 }}>
        ...
        <form onSubmit={savePayrollInfo} style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <label>Petpooja employee code
            <input value={petpoojaCode} onChange={e => setPetpoojaCode(e.target.value)} style={{ display: 'block' }} />
          </label>
